...
@if ($component->collapsingColumnsAreEnabled() && $component->hasCollapsedColumns())
    ...
    @if ($theme === 'tailwind')
        <tr wire:key="row-{{ $rowIndex }}-collapsed-contents"
            ...
            class="hidden bg-white dark:bg-gray-700 dark:text-white md:hidden">
            <td class="px-4 pb-2 pt-4" colspan="{{ $colspan }}">
                <div>
                    @foreach ($columns as $colIndex => $column)
                        ...
                        <p
                            class="@if ($column->shouldCollapseOnMobile()) sm:hidden @endif @if ($column->shouldCollapseOnTablet()) md:hidden @endif mb-2 block">
            ...
            <td class="p-2 pt-3" colspan="{{ $colspan }}">
                <div>
                    @foreach ($columns as $colIndex => $column)
                        ...
                        <p
                            class="d-block @if ($column->shouldCollapseOnMobile()) d-sm-none @endif @if ($column->shouldCollapseOnTablet()) d-md-none @endif mb-2">
                            <strong>{{ $column->getTitle() }}</strong>: {{ $column->renderContents($row) }}
                        </p>
                    @endforeach
                </div>
